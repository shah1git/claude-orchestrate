...
Входов в оркестрацию два (ADR-0004): полный `/orchestrate` — сырые задачи и
разбирательства, подготовительная фаза (грилл → спека → тикеты) в его же
сессии; тонкий `/orchestrate-frontier` — исполнение готового фронтира тикетов,
нарезанного владельцем в сольной сессии. Этот файл читают все агент-CLI,
которым раздаётся работа — не только Claude Code, но и codex- и gemini-лейны
(config v9 `promoted`).
