...
class BookingEngine:
    ...
    def create_booking(self, kit_id: str, serials: list[str], start_date: date, day_plan: tuple[DayType, ...]) -> Booking:
        ...
        # Audit trail records the undiscounted list price of the kit (field
        # rate x days per instrument), for finance to see gross exposure.
        audit_total = sum((tariff.RATE_BY_CLASS[i.instrument_class] * len(day_plan) for i in instruments), Decimal("0.00"))
        for serial in serials:
            self._active_by_serial[serial] = booking_id
        self._ledger.append("booking_created", booking_id, {"serials": list(serials), "total_cost": str(audit_total)})
        return booking
