...
def day_coefficient(day_type: DayType, is_weekend: bool) -> Decimal:
    """Return the pricing coefficient for a single day of a booking.

    The coefficient is ``COEF_FIELD_DAY`` or ``COEF_OFFICE_DAY`` depending
    on ``day_type``, multiplied by ``COEF_WEEKEND_EXTRA`` if ``is_weekend``
    is true. This is the same per-day coefficient logic ``compute_total``
    used to compute inline; it is exposed here so other modules (and
    tests) can query a single day's coefficient without re-deriving the
...
def compute_total(instrument_class: InstrumentClass, start_date: date, day_plan: tuple[DayType, ...]) -> Decimal:
    ...
    for offset, day_type in enumerate(day_plan):
        ...
        # Saturday=5, Sunday=6 in Python's date.weekday() numbering.
        is_weekend = current_date.weekday() >= 5
        subtotal += base_rate * day_coefficient(day_type, is_weekend)
