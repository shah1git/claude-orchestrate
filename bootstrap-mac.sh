...
# Готовность чекаута определяет сам Git. Проверять `-d "${dir}/.git"` нельзя: в
# связанном worktree (`git worktree add`) `.git` — файл-указатель, а не каталог,
# и такая проверка отправляла bootstrap клонировать поверх непустого каталога,
# где он падал с fatal. Сравнение с вершиной рабочего дерева сохраняет прежнюю
# защиту: подкаталог чужого репозитория чекаутом по-прежнему не считается.
clone_or_pull() {
  local dir="$1" repo="$2" parent top entries
  top="$(git -C "${dir}" rev-parse --show-toplevel 2>/dev/null || true)"
  if [ -n "${top}" ] && [ "${top}" = "$(cd "${dir}" 2>/dev/null && pwd -P)" ]; then
    git -C "${dir}" pull --ff-only && ok "обновлён ${dir}"
    return
  fi

  if [ -e "${dir}" ]; then
    [ -d "${dir}" ] || die "${dir} существует и не является каталогом"
    shopt -s nullglob dotglob
    entries=("${dir}"/*)
    shopt -u nullglob dotglob
    [ "${#entries[@]}" -eq 0 ] \
      || die "${dir} существует, но не является git checkout и не пуст — уберите каталог или запустите bootstrap из чекаута"
  fi
