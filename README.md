...
```text
runId · revision · stateHash · entry · phase · nextActions · configFingerprint · manifestFingerprint · tokensSpent
```

`entry` присутствует в Карточке всегда и называет допуск Прогона — `full`,
`frontier` либо `sweep`; от него зависит, какие переходы и какой payload законны.
Имя Прогона составное: `pocock-<unix-время старта>-<модель Слота skilled>-<uuid12>`,
где сегмент модели — только метка (он опускается, если у Слота нет witness), а
полный перечень моделей Слотов живёт в состоянии, не в имени.
...
(`tokensSpent` Карточки — сумма по всем улаженным попыткам, включая Линзы и
провалы) принадлежат Прогону и не сбрасываются новой сессией; потолка расхода нет —
