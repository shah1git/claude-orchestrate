...
telemetry-from-envelope path. An Orca-terminal-backed substrate ran alongside
`SubprocessSubstrate` for a time (fully implemented 2026-07-22, its own live
round-trip verified) but was withdrawn outright — the owner's decision to
drop Orca from the project (ADR-0008) — leaving `SubprocessSubstrate` as the
sole substrate.
...
model alias `kimi-code/k3` — a bare `k3` is a config.invalid). Still unverified
live: claude-print. The parsing/argv/strip logic stays unit-tested against
fixtures; the live round-trips are integration checks, now run at least once
per adapter except claude-print.
"""
