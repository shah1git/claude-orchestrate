...
[единый протокол исполнения](references/execution-protocol.md) и следуйте ему без
исключений с этой самой мутации: «Дисциплина состояния» и «Payload переходов»
действуют уже в фазе `preparation`, «Исполнительный цикл» — после публикации
Тикетов. Прямой путь, не создающий Прогона, в протокол не входит.
...
После `publish_tickets` следуйте только
[единому протоколу исполнения](references/execution-protocol.md). Это
единственный нормативный источник для дисциплины состояния, запечатанной
раздачи, host-witness UI-доказательства, pre-gate, Линз, приёмки,
`continue_wave`, синтеза и терминальных переходов; не воспроизводите и не
расширяйте его policy в этой Голове.
