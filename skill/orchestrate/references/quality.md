...
> **HISTORICAL METHOD REFERENCE SINCE ADR-0009 (2026-08-01).** The active
> acceptance contract is generated by `omp_runtime.py` from `config.yaml` and
> the sealed ticket; [delegation.md](delegation.md) documents that interface.
> Thresholds, channels, commands, and legacy state transitions below are
> provenance, not instructions for an active public Head. The wave-level lens
> clarification in §3 and §3b records the current 2026-08-02 contract.
